...
namespace pvd
{
	...
	{
	public:
		static std::shared_ptr<WebVTTProvider> Create(const cfg::Server &server_config, const std::shared_ptr<MediaRouterInterface> &router);

		explicit WebVTTProvider(const cfg::Server &server_config, const std::shared_ptr<MediaRouterInterface> &router);
		~WebVTTProvider() override;
